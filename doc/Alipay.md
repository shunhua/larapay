...
1. 判断设备选择实例化
	...
	```php
	$alipay = new \Shunhua\Larapay\Alipay\WebPay($config);
	```
	
	Wap(手机网页)
	
	```php
	$alipay = new \Shunhua\Larapay\Alipay\WapPay($config);
	```
	
	App
	
	```php
	$alipay = new \Shunhua\Larapay\Alipay\AppPay($config);
	```
...
4. 处理支付宝回调
	...
	```php
	$alipay = new \Shunhua\Larapay\Alipay\Notify($config);
...
```php
$alipay = new \Shunhua\Larapay\Alipay\Transfer($config);
