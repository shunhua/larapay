...
```php
$wechatpay = new \Shunhua\Larapay\Wechatpay\MpPay($config);  
```

App

```php
$wechatpay = new \Shunhua\Larapay\ Wechatpay\AppPay($config);
```

H5

```php
$wechatpay = new \Shunhua\Larapay\ Wechatpay\H5Pay($config);
```

Native

```
$wechatpay = new \Shunhua\Larapay\ Wechatpay\NativePay($config);
```
...
```php
$wechatpay = new \Shunhua\Larapay\Wechatpay\Notify($config);
...
```php
$wechatpay_refund = new \Shunhua\Larapay\Wechatpay\Refund($config);
...
$payment = new \Shunhua\Larapay\Wechatpay\Transfer($config);
$params = array(
...
* 企业付款到银行卡
	...
	1. 调用获取RSA公钥API获取RSA公钥，落地成本地文件，假设为rsa_public.pem
		...
		$wechatpay = new \Shunhua\Larapay\Wechatpay\Tools($config);
	...
	3. 执行
		...
		```php
		$config = config('payment.wechatpay.che');
		$wechatpay = new \Shunhua\Larapay\Wechatpay\Transfer($config);
...
```php
$config = config('payment.wechatpay.mp');
$payment = new \Shunhua\Larapay\Wechatpay\RedPack($config);
